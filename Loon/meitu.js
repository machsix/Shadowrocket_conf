...
[task_local]
30 20 * * * https://raw.githubusercontent.com/machsix/Shadowrocket_conf/main/Loon/meitu.js, tag=美图鉴赏, enabled=true, img-url=https://github.com/Koolson/Qure/blob/master/IconSet/Want_Want.png
...
function render(list, title) {
  return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${title}</title><style>*{margin:0;padding:0;}h1{padding:10px;font-size:1em;}ul{display:flex;flex-wrap:wrap;flex-direction:row;justify-content:center;align-items:center;}li{list-style:none;}img{width:100%;}</style></head><body><h1>${title}</h1><ul>${list
    ...
    .join("")}</ul><p>Action</p><form><input type="radio" id="update" name="action" value="update"><label for="update">Update</label><br><input type="radio" id="clear" name="action" value="clear"><label for="update">Clear</label><br><input type="submit" value="Submit"></form></body></html>`;
}
const main = async () => {
  try {
    await showNotice();
